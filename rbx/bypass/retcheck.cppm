...
export module rbx.bypass.retcheck;
...
void fix_calls(DWORD new_func, DWORD orig_func, DWORD func_len);

bool is_prolog(BYTE* addr)
{
	return addr[0] == 0x55 && addr[1] == 0x8B && addr[2] == 0xEC;
}

BYTE* get_func_end(BYTE* funcaddr) //terrible
{
...
const int max_alloc = 1024 * 1024;

bool disable_retcheck(DWORD new_func, DWORD func_len)
{
...
DWORD unprotect(BYTE* funcaddr)
{
	static int total_alloc;
	static std::unordered_map<DWORD, DWORD> cache;

	auto cached_func = cache.find((DWORD)funcaddr);
	if (cached_func != cache.end())
		return (DWORD)(cached_func->second);
...
namespace rbx::bypass {
	export inline std::uintptr_t retcheck(std::uintptr_t address) {
		return unprotect(reinterpret_cast<BYTE*>(address));
	}
}
